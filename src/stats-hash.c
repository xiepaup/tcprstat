...
//#include "config.h"   //comment by xiean@2016-11-20

#define INITIAL_HASH_SZ     2053
#define MAX_LOAD_PERCENT    65

/*
 * 定义每个数据包的 IP:Port:Time
 * */
struct session {
...
int hash_get(struct hash *hash, uint32_t laddr, uint32_t raddr,
             uint16_t lport, uint16_t rport, struct timeval *result) {
...
int hash_get_rem(struct hash *hash, uint32_t laddr, uint32_t raddr,
                 uint16_t lport, uint16_t rport, struct timeval *result) {
...
int hash_clean(struct hash *hash, unsigned long min) {
    ...
    for (i = 0; i < hash->sz; i++) {
        ...
        for (session = hash->sessions + i; session->next; session = session->next) {
            if (session->next->tv.tv_sec * 1000000 + session->next->tv.tv_usec < min) {
                struct session *next;
...
static unsigned long hash_fun(uint32_t laddr, uint32_t raddr, uint16_t lport, uint16_t rport) {
    ...
    ret ^= ((uint64_t) lport << 48) | ((uint64_t) rport << 32) | ((uint64_t) lport << 16) | rport;
